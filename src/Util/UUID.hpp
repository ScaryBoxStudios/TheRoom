/*********************************************************************************************************************/
/*                                                  /===-_---~~~~~~~~~------____                                     */
/*                                                 |===-~___                _,-'                                     */
/*                  -==\\                         `//~\\   ~~~~`---.___.-~~                                          */
/*              ______-==|                         | |  \\           _-~`                                            */
/*        __--~~~  ,-/-==\\                        | |   `\        ,'                                                */
/*     _-~       /'    |  \\                      / /      \      /                                                  */
/*   .'        /       |   \\                   /' /        \   /'                                                   */
/*  /  ____  /         |    \`\.__/-~~ ~ \ _ _/'  /          \/'                                                     */
/* /-'~    ~~~~~---__  |     ~-/~         ( )   /'        _--~`                                                      */
/*                   \_|      /        _)   ;  ),   __--~~                                                           */
/*                     '~~--_/      _-~/-  / \   '-~ \                                                               */
/*                    {\__--_/}    / \\_>- )<__\      \                                                              */
/*                    /'   (_/  _-~  | |__>--<__|      |                                                             */
/*                   |0  0 _/) )-~     | |__>--<__|     |                                                            */
/*                   / /~ ,_/       / /__>---<__/      |                                                             */
/*                  o o _//        /-~_>---<__-~      /                                                              */
/*                  (^(~          /~_>---<__-      _-~                                                               */
/*                 ,/|           /__>--<__/     _-~                                                                  */
/*              ,//('(          |__>--<__|     /                  .----_                                             */
/*             ( ( '))          |__>--<__|    |                 /' _---_~\                                           */
/*          `-)) )) (           |__>--<__|    |               /'  /     ~\`\                                         */
/*         ,/,'//( (             \__>--<__\    \            /'  //        ||                                         */
/*       ,( ( ((, ))              ~-__>--<_~-_  ~--____---~' _/'/        /'                                          */
/*     `~/  )` ) ,/|                 ~-_~>--<_/-__       __-~ _/                                                     */
/*   ._-~//( )/ )) `                    ~~-'_/_/ /~~~~~~~__--~                                                       */
/*    ;'( ')/ ,)(                              ~~~~~~~~~~                                                            */
/*   ' ') '( (/                                                                                                      */
/*     '   '  `                                                                                                      */
/*********************************************************************************************************************/
#ifndef _UUID_HPP_
#define _UUID_HPP_

#include <string>

/**
 * rUUID - A class for the representation of Universal Unique IDentifiers
 */
class rUUID
{
    public:
        // Default constructor
        rUUID();

        // Copy constructor
        rUUID(const rUUID& rhs);

        // Assignment operator
        void operator=(const rUUID& rhs);

        // Equality operator
        bool operator==(const rUUID& rhs) const;

        // Leq than operator
        bool operator<(const rUUID& rhs) const;

        // Returns the stored string representation of the UUID
        const std::string& ToString() const;

        // TODO: Remove this when scene loading is 100% complete
        void SetUuid(const std::string& newId);

        // Returns the stored string representation of the UUID in a c string form
        const char* c_str() const;

        // Implements the cast operator char* so we can use the UUID object as a C string
        operator const char*() const;

        // Side constructor
        friend bool StringToUUID(const char* str, rUUID& uuid);

    private:
        // Stores the generated uuid
        std::string mUUID;
};

bool StringToUUID(const char* str, rUUID& uuid);

#endif // ! _UUID_HPP_
