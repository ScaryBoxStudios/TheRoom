...
As stated before, light arriving in a point may have been reflected or refracted from another point int the scene.
Offline algorithms may be able to capture that effect but in realtime applications this computation is impossible.
...
A technique called Image Base Lighting (IBL) is one solution that addresses that problem. Firstly, the object's environment is
...
environment map as a light source. They result can also be stored in cubemaps that can be queried later and be used to calculate the
environmental light contribution on a fragment.
...
The environmental diffuse light contribution can be captured on an Irradiance Map. The Irradiance map can be generated
